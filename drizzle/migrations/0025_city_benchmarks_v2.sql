...
BEGIN
  ...
  pr AS (
    SELECT id,
      (percent_rank() OVER (ORDER BY res_per_week)
       ...
       + percent_rank() OVER (ORDER BY cancel_rate DESC)) / 6.0 AS score
    FROM m2
  ),
  ranked AS (
  ...
  agg AS (
    SELECT
      AVG(res_per_week) AS res_per_week, percentile_cont(0.75) WITHIN GROUP (ORDER BY res_per_week) AS res_per_week_top,
      AVG(avg_party) AS avg_party, percentile_cont(0.75) WITHIN GROUP (ORDER BY avg_party) AS avg_party_top,
      AVG(cancel_rate) AS cancel_rate, percentile_cont(0.25) WITHIN GROUP (ORDER BY cancel_rate) AS cancel_rate_top,
      AVG(avg_price) AS avg_price, percentile_cont(0.75) WITHIN GROUP (ORDER BY avg_price) AS avg_price_top,
      AVG(reach_per_event) AS reach, percentile_cont(0.75) WITHIN GROUP (ORDER BY reach_per_event) AS reach_top,
      AVG(engagement) AS engagement, percentile_cont(0.75) WITHIN GROUP (ORDER BY engagement) AS engagement_top,
      AVG(followers) AS followers, percentile_cont(0.75) WITHIN GROUP (ORDER BY followers) AS followers_top,
      AVG(guestlist_fill) AS guestlist_fill, percentile_cont(0.75) WITHIN GROUP (ORDER BY guestlist_fill) AS guestlist_fill_top
    FROM peers_m
  ),
