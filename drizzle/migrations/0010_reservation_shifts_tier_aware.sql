-- Effective bookable shifts for a business on a given weekday.
-- No silent fallback: if the business has not configured schedules, no shifts.
-- Básico plan is limited to a single shift per day (earliest start).
CREATE OR REPLACE FUNCTION public.get_business_shifts(_business_id uuid, _weekday int)
RETURNS TABLE(start_time time, end_time time)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $fn$
  SELECT sc.start_time, sc.end_time
  FROM public.reservation_schedules sc
  WHERE sc.business_id = _business_id
    AND sc.weekday = _weekday
    AND NOT sc.is_closed
  ORDER BY sc.start_time
  LIMIT CASE
    WHEN COALESCE(public.get_business_public_tier(_business_id), 'basico') = 'basico' THEN 1
    ELSE 100
  END
$fn$;

GRANT EXECUTE ON FUNCTION public.get_business_shifts(uuid, int) TO anon, authenticated, service_role;
...
BEGIN
  ...
  FOR s IN SELECT * FROM public.get_business_shifts(_business_id, dow)
  LOOP
    slot := s.start_time;
