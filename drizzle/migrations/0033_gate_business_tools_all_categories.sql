-- 1) Every business account (any category) gets a subscription row, not just food ones
...
-- Backfill rows for existing non-food business accounts
INSERT INTO public.business_subscriptions (business_id, tier, status, activation_method)
SELECT p.id, 'basico', 'pending_activation', 'manual'
FROM public.profiles p
WHERE p.is_business = true
ON CONFLICT (business_id) DO NOTHING;

-- 2) Server-side paywall: menu / reservations can only be switched on with an active plan
CREATE OR REPLACE FUNCTION public.enforce_business_plan_for_tools()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
BEGIN
  IF (COALESCE(NEW.menu_enabled, false) AND NOT COALESCE(OLD.menu_enabled, false))
     OR (COALESCE(NEW.reservations_enabled, false) AND NOT COALESCE(OLD.reservations_enabled, false)) THEN
    IF NOT public.has_active_business_plan(NEW.id) THEN
      RAISE EXCEPTION 'Necesitás un plan activo para activar esta función';
    END IF;
  END IF;
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS enforce_business_plan_for_tools_trigger ON public.profiles;
CREATE TRIGGER enforce_business_plan_for_tools_trigger
  BEFORE UPDATE OF menu_enabled, reservations_enabled ON public.profiles
  FOR EACH ROW EXECUTE FUNCTION public.enforce_business_plan_for_tools();

REVOKE EXECUTE ON FUNCTION public.enforce_business_plan_for_tools() FROM PUBLIC, anon, authenticated;

-- 3) Reservations require an active plan at booking time too
...
BEGIN
  ...
  IF _reservation_id IS NOT NULL THEN
    UPDATE reservations SET
      ...
      duration_minutes = turn,
      status = 'confirmed',
      updated_at = now()
    WHERE id = _reservation_id AND user_id = uid
    RETURNING id INTO res_id;
    IF res_id IS NULL THEN RAISE EXCEPTION 'No se encontró esta reserva'; END IF;
    DELETE FROM reservation_tables WHERE reservation_id = res_id;
  ELSE
    INSERT INTO reservations (business_id, user_id, reservation_date, reservation_time, party_size, notes, duration_minutes, status)
    VALUES (_business_id, uid, _date, _time, _party_size, _notes, turn, 'confirmed')
    RETURNING id INTO res_id;
  END IF;

  IF has_tables THEN
    INSERT INTO reservation_tables (reservation_id, table_id)
    SELECT res_id, unnest(chosen)
    ON CONFLICT DO NOTHING;
  END IF;

  IF _guest_ids IS NOT NULL THEN
    INSERT INTO reservation_guests (reservation_id, user_id)
    SELECT res_id, g FROM unnest(_guest_ids) g
    ON CONFLICT DO NOTHING;
  END IF;
