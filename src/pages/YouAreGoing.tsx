...
const YouAreGoing = () => {
  ...
  return (
    ...
    >
      ...
      >
        {/* You Are Going */}
        <p className="text-sm font-medium uppercase tracking-widest opacity-80">You Are Going</p>

        {/* Event Creator's Display Name */}
        <h1 className="font-bold mt-4 font-brand text-primary text-sm">
          {event.creator?.full_name || event.creator?.username || "Host"}
        </h1>

        {/* Event Title */}
        <h2 className="font-semibold mt-2 opacity-90 text-primary text-5xl">{event.title}</h2>
      ...
      <Dialog open={showQR} onOpenChange={setShowQR}>
        <DialogContent className="bg-background text-foreground max-w-xs rounded-2xl">
          ...
          <div className="flex flex-col items-center py-6">
            ...
            <p className="text-sm text-muted-foreground mt-4 text-center">Show this at the entrance</p>
          </div>
        </DialogContent>
      </Dialog>
    </motion.div>
