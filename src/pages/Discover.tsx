...
const Discover = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from("guestlist_entries").select("event_id, user_id").in("event_id", events.map(e => e.id));
      if (error) throw error;
  ...
  const activeFilterCount = (filters.dateFilter !== "all" ? 1 : 0) + filters.categories.length + (filters.maxDistance !== null ? 1 : 0) + (filters.hasGuestlistOnly ? 1 : 0) + (filters.friendsGoingOnly ? 1 : 0);
  ...
  return <AppLayout>
      ...
      <div className="relative h-[calc(100vh-80px)] bg-secondary">
        ...
        <MapView events={filteredEvents} onMarkerClick={handleMarkerClick} selectedEventId={selectedEvents[currentSlide]?.id} onGeolocationSuccess={handleGeolocationSuccess} onGeolocationError={handleGeolocationError} />
        ...
        <div className="absolute top-0 left-0 right-0 z-40 safe-top px-4 py-4">
          <div className="flex gap-2 py-[26px]" ref={searchContainerRef}>
            <div className="relative flex-1">
              ...
              <Input placeholder="Search events, people..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onFocus={() => setIsSearchFocused(true)} className="pl-10 pr-10 bg-card/90 backdrop-blur-md border-border/50" />
              {searchQuery && <button onClick={handleClearSearch} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground z-10">
                  <X className="w-4 h-4" />
                </button>}

              {/* Search Results Dropdown */}
              <AnimatePresence>
                {showSearchDropdown && <motion.div initial={{
                opacity: 0,
                y: -10
              }} animate={{
                opacity: 1,
                ...
                duration: 0.15
                    ...
                    <div className="flex border-b border-border/50">
                      <button onClick={() => setSearchTab("events")} className={cn("flex-1 px-4 py-3 text-sm font-medium transition-colors flex items-center justify-center gap-2", searchTab === "events" ? "text-primary border-b-2 border-primary bg-primary/5" : "text-muted-foreground hover:text-foreground")}>
                        <MapPin className="w-4 h-4" />
                        Events ({filteredEvents.length})
                      </button>
                      <button onClick={() => setSearchTab("people")} className={cn("flex-1 px-4 py-3 text-sm font-medium transition-colors flex items-center justify-center gap-2", searchTab === "people" ? "text-primary border-b-2 border-primary bg-primary/5" : "text-muted-foreground hover:text-foreground")}>
                    ...
                    <div className="overflow-y-auto max-h-[calc(60vh-48px)]">
                      {searchTab === "events" ? <div className="p-2">
                          {filteredEvents.length > 0 ? filteredEvents.slice(0, 10).map(event => <button key={event.id} onClick={() => handleMarkerClick([event])} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-accent/50 transition-colors text-left">
                                <div className="w-12 h-12 rounded-lg bg-secondary overflow-hidden flex-shrink-0">
                                  <img src={event.image_url || "/placeholder.svg"} alt={event.title || "Event"} className="w-full h-full object-cover" />
                                </div>
                                ...
                                {event.distance !== null && <span className="text-xs text-muted-foreground flex-shrink-0">
                                    {formatDistance(event.distance)}
                                  </span>}
                              </button>) : <div className="py-8 text-center text-muted-foreground">No events found</div>}
                        </div> : <div className="p-2">
                          {isLoadingUsers ? <div className="py-8 text-center text-muted-foreground">Searching...</div> : searchedUsers.length > 0 ? searchedUsers.map(user => <UserSearchResultCard key={user.id} user={user} onClick={handleUserClick} />) : <div className="py-8 text-center text-muted-foreground">No people found</div>}
                        </div>}
                    </div>
                  </motion.div>}
              </AnimatePresence>
            </div>
            <Button variant="secondary" size="icon" className="bg-card/90 backdrop-blur-md border-border/50 relative" onClick={() => setIsFilterOpen(true)}>
              <SlidersHorizontal className="w-4 h-4" />
              {activeFilterCount > 0 && <span className="absolute -top-1 -right-1 w-5 h-5 bg-primary text-primary-foreground text-[10px] font-bold rounded-full flex items-center justify-center">
                  {activeFilterCount}
                </span>}
            </Button>
          </div>
        ...
        {/* No events message */}
        {eventsWithLocation.length === 0 && <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
            <div className="text-center bg-card/80 backdrop-blur-md rounded-2xl p-6 mx-4">
              ...
              <p className="text-sm text-muted-foreground max-w-xs">
                {searchQuery || activeFilterCount > 0 ? "Try adjusting your search or filters" : "Events with locations will appear here as pins"}
              </p>
            </div>
          </div>}
        ...
        <AnimatePresence>
          {selectedEvents.length > 0 && <motion.div initial={{
          y: "100%",
          opacity: 0
        }} animate={{
          y: 0,
          ...
          damping: 25,
          stiffness: 300
        }} className="absolute bottom-20 left-4 right-4 z-50" onClick={e => e.stopPropagation()}>
              <div className="relative">
                <Button variant="secondary" size="icon" className="absolute -top-2 -right-2 z-10 w-8 h-8 rounded-full bg-card shadow-elevated" onClick={handleCloseEventCard}>
                  <X className="w-4 h-4" />
                </Button>

                {/* Distance badge - shows current event's distance */}
                {selectedEvents[currentSlide]?.distance !== null && <div className="absolute -top-2 -left-2 z-10 px-2 py-1 bg-primary text-primary-foreground text-xs font-medium rounded-full shadow-elevated">
                    {formatDistance(selectedEvents[currentSlide].distance!)}
                  </div>}

                {/* Single event or carousel */}
                {selectedEvents.length === 1 ? <EventCard {...eventToCardProps(selectedEvents[0])} /> : <div className="space-y-3">
                    <Carousel className="w-full" setApi={setCarouselApi}>
                      <CarouselContent>
                        {selectedEvents.map(event => <CarouselItem key={event.id}>
                    ...
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <div className="flex justify-center items-center gap-1.5">
                        {selectedEvents.map((_, index) => <button key={index} onClick={() => carouselApi?.scrollTo(index)} className={cn("w-2 h-2 rounded-full transition-all duration-200", index === currentSlide ? "bg-primary w-4" : "bg-muted-foreground/40 hover:bg-muted-foreground/60")} />)}
                      </div>
                    </div>
                  </div>}
              </div>
          ...
          <DrawerContent className="max-h-[70vh]">
            <div className="px-4 pb-8 overflow-y-auto">
              ...
              <div className="columns-2 gap-3 pb-4">
                {filteredEvents.map((event, index) => <div key={event.id} className="relative break-inside-avoid mb-3">
                    {/* Distance badge */}
                    {event.distance !== null && <div className="absolute top-2 right-2 z-10 px-2 py-0.5 bg-background/80 backdrop-blur-sm text-foreground text-[10px] font-medium rounded-full">
                        {formatDistance(event.distance)}
                      </div>}
        ...
        {/* Filter Sheet */}
        <FilterSheet open={isFilterOpen} onOpenChange={setIsFilterOpen} filters={filters} onApplyFilters={handleApplyFilters} />
      </div>
    </AppLayout>;
};
