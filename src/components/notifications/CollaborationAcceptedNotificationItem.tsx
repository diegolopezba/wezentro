...
export const CollaborationAcceptedNotificationItem = ({ notification, index, onRead, onClick }: Props) => {
  const extractedUsername = notification.body?.match(/@(\w+)/)?.[1];
  const { data: event } = useEvent(notification.entity_id || undefined);
  const { data: collaboratorProfile } = useQuery({
    ...
    >
      <div className="relative">
        <Avatar className="w-10 h-10 shrink-0">
          <AvatarImage src={collaboratorProfile?.avatar_url || DEFAULT_AVATAR} />
          <AvatarFallback />
        </Avatar>
