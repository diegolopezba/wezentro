...
const MapView: React.FC<MapViewProps> = ({
  ...
  const eventsGeoJSON = useMemo(() => {
    const features = events.filter(event => event.latitude && event.longitude).map(event => ({
  ...
  useEffect(() => {
    ...
    map.current.on('load', () => {
      ...
      map.current.addLayer({
        ...
        paint: {
          'circle-color': ['step', ['get', 'point_count'], 'hsl(351, 100%, 45%)',
      ...
      map.current.addLayer({
        ...
        filter: ['!', ['has', 'point_count']],
        paint: {
          'circle-color': ['case', ['get', 'isTonight'], 'hsl(351, 100%, 55%)',
    ...
    map.current.on('click', 'unclustered-point', e => {
      ...
      const eventsAtLocation = events.filter(ev => ev.latitude && ev.longitude && Math.abs(ev.latitude - clickedCoords[1]) < tolerance && Math.abs(ev.longitude - clickedCoords[0]) < tolerance);
  ...
  // Loading state
  if (tokenLoading) {
    return <div className="absolute inset-0 flex items-center justify-center bg-secondary">
  ...
  return <div className="absolute inset-0">
      ...
      <Button variant="secondary" size="sm" onClick={handleRefreshLocation} className="absolute bottom-24 right-3 z-10 shadow-lg gap-2">
