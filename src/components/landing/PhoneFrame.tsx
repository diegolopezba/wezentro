...
export const PhoneFrame = ({ src, alt, className }: { src: string; alt: string; className?: string }) => (
  <div
    className={cn("mx-auto w-full max-w-[250px] rounded-[42px] border border-black bg-black p-2 shadow-2xl", className)}
  >
...
export const MediaSplit = ({
  ...
  <div className="grid items-center gap-10 lg:grid-cols-2">
    <Reveal className={cn(reverse && "lg:order-2")}>
      <h2 className="font-brand text-3xl font-semibold leading-tight tracking-tight sm:text-5xl">{title}</h2>
      <p className="mt-4 max-w-md text-base leading-relaxed text-muted-foreground sm:text-lg">{line}</p>
      {children}
    </Reveal>
...
export const LabelStrip = ({ items }: { items: string[] }) => (
  <div className="mt-7 flex flex-wrap gap-2">
    {items.map((item) => (
      <span key={item} className="rounded-full border border-border bg-card px-3.5 py-2 text-sm font-medium">
        {item}
      </span>
    ))}
  </div>
...
export const ScreenStrip = ({ shots }: { shots: { src: string; alt: string }[] }) => (
