    ...
    <div className="flex items-center gap-3 py-3 border-b border-border last:border-b-0">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {item.price !== null && (
            <span className="text-sm text-muted-foreground">Bs. {item.price.toFixed(2)}</span>
          )}
        </div>
        {item.description && (
...
export const MenuEditor = () => {
  ...
  const handleMoveCategory = async (index: number, direction: "up" | "down") => {
    ...
    const updates = categories.map((cat, i) => ({ id: cat.id, display_order: i }));
    try {
    ...
    <>
      <div className="rounded-xl bg-card border border-border p-4">
        {menu && (
          <div className="flex items-center justify-end gap-2 mb-4">
            <Button size="sm" variant="outline" onClick={() => handleOpenCategoryDialog()} className="gap-1">
              <FolderPlus className="w-4 h-4" />
              Categoría
            </Button>
            <Button size="sm" onClick={() => handleOpenItemDialog()} className="gap-1">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-muted-foreground mb-4">Aún no tienes un menú</p>
            <Button onClick={handleCreateMenu} disabled={createMenuMutation.isPending} className="gap-2">
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            <UtensilsCrossed className="w-12 h-12 text-muted-foreground mb-4" />
            <p className="text-muted-foreground mb-4">Tu menú está vacío</p>
      ...
      <Sheet open={isItemDialogOpen} onOpenChange={setIsItemDialogOpen}>
        <SheetContent side="bottom" className="light-sheet h-auto max-h-[90vh] rounded-t-3xl flex flex-col">
          <SheetHeader className="pb-2">
            <SheetTitle>{editingItem ? "Editar Item" : "Agregar Item"}</SheetTitle>
          </SheetHeader>
      ...
      <Sheet open={isCategoryDialogOpen} onOpenChange={setIsCategoryDialogOpen}>
        <SheetContent side="bottom" className="light-sheet h-auto rounded-t-3xl flex flex-col">
          <SheetHeader className="pb-2">
            <SheetTitle>{editingCategory ? "Editar Categoría" : "Agregar Categoría"}</SheetTitle>
          </SheetHeader>
