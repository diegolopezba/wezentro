...
4. **Fee safety and diagnostics**
   - Keep the visible 1% processing line and total calculation, but add a small configurable minimum/rounding buffer if Qhantuy’s production fee requires more than the nominal percentage on low-value payments.
   - Log a sanitized checkout breakdown and preserve Qhantuy’s response message/code, without customer or credential data, so future provider rejections can be diagnosed directly.
   - Surface the provider’s actionable rejection message instead of replacing it with a generic QR error.
